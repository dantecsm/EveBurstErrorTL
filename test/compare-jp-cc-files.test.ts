...
interface CompareStats {
  ...
  identical: number;
  different: number;
  missingInJPCC: string[];
  missingInOldJPCC: string[];
}
...
function compareDirectories(dir1: string, dir2: string): {
  ...
  for (const file of allFiles) {
    ...
    if (!exists1) {
      stats.missingInJPCC.push(file);
      continue;
    }

    if (!exists2) {
      stats.missingInOldJPCC.push(file);
      continue;
    }
 ...
 */
function main() {
  const oldJPCCDir = "ref/OLD_JP_CC";
  const jpCCDir = "data/JP CC";

  console.log("比较 OLD_JP_CC 和 JP CC 文件...\n");

  const { stats, details } = compareDirectories(oldJPCCDir, jpCCDir);
  ...
  // 显示缺失文件
  if (stats.missingInJPCC.length > 0) {
    console.log("\n仅在 OLD_JP_CC 中存在的文件:");
    for (const file of stats.missingInJPCC) {
      console.log(`  - ${file}`);
    }
  }

  if (stats.missingInOldJPCC.length > 0) {
    console.log("\n仅在 JP CC 中存在的文件:");
    for (const file of stats.missingInOldJPCC) {
      console.log(`  - ${file}`);
    }
  }
  ...
  if (stats.missingInJPCC.length > 0) {
    console.log(`  ⚠ 仅在 OLD_JP_CC 中: ${stats.missingInJPCC.length}`);
  }
  if (stats.missingInOldJPCC.length > 0) {
    console.log(`  ⚠ 仅在 JP CC 中: ${stats.missingInOldJPCC.length}`);
  }
  ...
  // 返回退出码
  if (stats.different > 0 || stats.missingInJPCC.length > 0 || stats.missingInOldJPCC.length > 0) {
    process.exit(1);
  }
  process.exit(0);
}
